/**
 * Convia Blueprint 2.0 — Quality, Delivery & Project Readiness Intelligence Engine (Phase 6)
 * 
...
export function evaluateQualityGates(rawGates = [], projectContext = {}, liveEvidence = {}) {
  ...
  const evaluatedGates = Array.from(gateMap.values()).map((gate) => {
    // Stage 1 (Requirements) & Stage 2 (Architecture): Automatically verified if Blueprint 2.0 contract is populated
    if (gate.id === 'GATE-01') {
